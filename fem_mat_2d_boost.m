...
% 
% VARARGOUT = FEM_MAT_2D_BOOST(V, N, XY_BOUND, MAT_TYPE, GQO)
...
% 
%   [Input Argument]
%       v - Function handle, the potential function
...
%       gqo - Vector, set it as [po qo] to set the order of Legendre Gauss
%             quadrature of local integral as po/qo corresponding to local
%             variables p/q (w.r.t. global variables x/y)
% 
%   [Ouput Argument]
%       A, B, M - Matrix, the Laplace, potential and mass matrices resp.
% 
% Details:
...
%      don't fall into any singular point, that is, if we choose [5 5] then
%      Gauss points will be on the singular line y = x, so we'd better use
%      different orders for x/y like [5 4]


function varargout = fem_mat_2d_boost(v, N, xy_bound, mat_type, gqo)
    % Input check
    if ~iscell(mat_type)
        mat_type = {mat_type};
    end
    ...
    ip = ones(N_n, 1); % Whether the n-th node is a interior point, 
