    ...
    if ~isempty(ref_line)
        for i = 1:size(ref_line, 1)
            if isinf(ref_line(i, 1)) % Vertical line
                ref_line_x = (ref_line(i, 2)*(x_max-x_min)+x_min)*...
                             ones(length(x), 1);
                ref_line_y = linspace(Y_min, Y_max, length(x));
            elseif ref_line(i, 1) == 0 % Horizontal line
                ref_line_x = linspace(x_min, x_max, length(x));
                ref_line_y = (ref_line(i, 2)*(Y_max-Y_min)+Y_min)*...
                             ones(length(x), 1);
            elseif ref_line(i, 1) > 0 % Positive slope
                intercept_min = Y_min-ref_line(i, 1)*x_max;
                intercept_max = Y_max-ref_line(i, 1)*x_min;
                intercept = ref_line(i, 2)*(intercept_max-intercept_min)...
                            +intercept_min;
                ref_line_x_min = max((Y_min-intercept)/ref_line(i, 1), ...
                                     x_min);
                ref_line_x_max = min((Y_max-intercept)/ref_line(i, 1), ...
                                     x_max);
                ref_line_y_min = max(ref_line(i, 1)*x_min+intercept, ...
                                     Y_min);
                ref_line_y_max = min(ref_line(i, 1)*x_max+intercept, ...
                                     Y_max);
                ...
                intercept_min = Y_min-ref_line(i, 1)*x_min;
                intercept_max = Y_max-ref_line(i, 1)*x_max;
                intercept = ref_line(i, 2)*(intercept_max-intercept_min)...
                            +intercept_min;
                ref_line_x_min = max((Y_max-intercept)/ref_line(i, 1), ...
                                     x_min);
                ref_line_x_max = min((Y_min-intercept)/ref_line(i, 1), ...
                                     x_max);
                ref_line_y_min = max(ref_line(i, 1)*x_max+intercept, ...
                                     Y_min);
                ref_line_y_max = min(ref_line(i, 1)*x_min+intercept, ...
                                     Y_max);
                ref_line_x = linspace(ref_line_x_min, ref_line_x_max, ...
                                      length(x));
                ref_line_y = linspace(ref_line_y_min, ref_line_y_max, ...
    ...
    if ~isempty(ref_line)
        for i = 1:size(ref_line, 1)
            if isinf(ref_line(i, 1))
                legend_cell{length(group_indices)+i} = '$Slope = \infty$';
            else
